#+begin_src latex
  ...
  with respect to the fragment start.
  ...
  $(12,13)$, $(17,17)$, and $(20,23)$ (Table~\ref{fig:cb}B).
...
#+begin_src latex
  ...
  data fields. If this key is new, we add a query. We also add a Blast
  hit.
#+end_src
...
#+begin_src latex
  ...
  relative to the query start. To simplify the later complementation
  step, we store the Blast hits in the same coordinates as the query. We
  do this by adding the query start and subtracting one.
#+end_src
...
#+begin_src latex
  ...
  except for the last interval, which is dealt with after the loop.
#+end_src
...
#+end_src
#+begin_src latex
  We carry out three tests. The first is on the small data set in
  ...
  analyzes both data files. The minimum interval length is always 1.
#+end_src
